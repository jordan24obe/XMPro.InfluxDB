...
{
    public class Listener : IAgent, IPollingAgent, IUsesVariable, IPublishesError
    {
        ...
        public IEnumerable<XMIoT.Framework.Attribute> GetOutputAttributes(string endpoint, IDictionary<string, string> parameters)
        {
            ...
            foreach (var row in FieldMapping.Rows)
            {
                ...
                string fieldType = row[FieldMappingColumnTypeConst].ToString();
                outputs.Add(new XMIoT.Framework.Attribute(fieldName, GetIoTType(fieldType)));
            }
            return outputs;
        }

        public Types GetIoTType(string type)
        {
            switch (type)
            {
                case "Int":
        ...
        public string CreateQuery(string start = null)
        {
            ...
            //add each field
            string fieldFilter = $"|> filter(fn: (r) =>";
            foreach (var row in FieldMapping.Rows)
            {
                string fieldName = row[FieldMappingColumnNameConst].ToString();
                fieldFilter += $" r[\"_field\"] == \"{fieldName}\" or";
            }
            fieldFilter = fieldFilter.Remove(fieldFilter.Length - 3) + ")\n"; //remove last or
            query += fieldFilter;
            return query;
        }
